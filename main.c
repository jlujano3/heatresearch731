...
double qto(int n, double h){
  double sum=M_PI/(2.0*h),tn=n*h;
  int i;
  
  for (i=1;i<n;i++){
    sum-=sqrt(tn-i*h)/(tn*sqrt(i*h));
  }
...
void solveState(int n, double tmax, double *r,double *qs){
  int i , j;
  double *us;
  //double *qs;
  ...
  //dro=(r[1]-r[0])/dt;
  //third order
  dro=(-11.0*r[0]/6.0+3.0*r[1]-3.0*r[2]/2.0+r[3]/3.0)/dt;
  duoVal=duo(r[0]);
...
void leg(int n, double x, double *y){
  ...
  if (n==1) return;
  for (i=2;i<=n;i++){
    y[i]=((2.0*(i-1.0)+1.0)*x*y[i-1]-(i-1.0)*y[i-2])/i;
  }
...
void compute_gradient(int N, int M, double tMax, double *a, double *qS, double *r,double *g){
...
void test_gradient(int N, int M, double tMax) {
  ...
  solveState(M,tMax,r,q);
  fcnl = compute_functional(M, tMax, q);
  compute_gradient(N, M, tMax, a, q, r,g);
...
void test_optimization(int numInts, int deg, int tmax, double h_0, int inner_itermax, int outer_itermax, int line_search_itermax){
  ...
  for (i=0; i<outer_itermax; i++) {     
    ...
    for (j=0; j<inner_itermax; j++) {  
      ...
      bdry_curve(deg, numInts, a, r, tmax);
      solveState(numInts,tmax,r,q);
      compute_gradient(deg, numInts, tmax, a, q, r,g);
...
int main(int nargs, char *argv[]){
  double tmax=1.0,dt,eps=1e-4,h_0=0.5,fcnl,fcnl1,fcnl2,Gd;
  int i,j,k,x,l,numInts=64,verbose=0,deg=10,M,inner_itermax=40,outer_itermax=10,line_search_itermax=10,job;
  double *r,*y,*z,*a,*q,*qa,*g;
...
if (job==0){
  ...
  test_optimization(numInts,deg,tmax,h_0,inner_itermax,outer_itermax,line_search_itermax);
}
